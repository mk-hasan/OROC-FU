...
public class orocGetMSBasedObject1 {

	public ArrayList<String> result(NodeList nodes, ArrayList<String> replacement_material_list, ArrayList<String> replacement_shape_list, ArrayList<Double> material_score, ArrayList<Double> shape_score) {
						...
						for (int j = 0; j < nodes.getLength(); j++) {
			            	...
			            	if(element.getAttribute("type").equalsIgnoreCase("primitive")) {
				            	...
				            	for(int o = 0 ; o < shape_list.getLength() ; o++) {
				            		Element line = (Element) shape_list.item(o);
				            		if(orocMain.getCharacterDataFromElement(line).toLowerCase().contains(choosenShape.toLowerCase())) {
				            			for(int o2 = 0 ; o2 < material_list.getLength() ; o2++) {
						            		...
						            		if(orocMain.getCharacterDataFromElement(line2).toLowerCase().contains(choosenMaterial.toLowerCase())) {
							            		...
							            		//count = count + 1;
							            		replacementObject.add((orocMain.getCharacterDataFromElement(line3)+"("+material_score.get(i)+","+shape_score.get(i)+")").toString());
						            		}
				            			}
					            	}
				            	}
